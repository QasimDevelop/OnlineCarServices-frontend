...
const ServiceStations = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (editingStation) {
        await api.put(
          `/api/accounts/service-stations/${editingStation.id}/`,
          form
        );
      ...
      setMessage(
        "Failed to save service station: " +
  ...
  const handleDelete = async (stationId) => {
    if (
      window.confirm("Are you sure you want to delete this service station?")
    ) {
      try {
          ...
          {stations.map((station) => (
            ...
            >
              <div className="p-6">
                ...
                {station.services_offered &&
                  station.services_offered.length > 0 && (
                    <div className="mb-4">
                ...
                <div className="flex gap-2">
                  <button
                    onClick={() =>
                      navigate(`/appointments?station=${station.id}`)
                    }
                  ...
                  <button
                    onClick={() => navigate(`/station-services/${station.id}`)}
